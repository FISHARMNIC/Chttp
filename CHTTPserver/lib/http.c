...
request_t http_parse_struct_from_req(char *str)
{
    ...
    */
    ...
    // char *method = malloc(len);
    // char *url = malloc(len);
    // char *headers = malloc(len);
    // char *data = malloc(len);
    // memset(method, 0, len);
    // memset(url, 0, len);
    // memset(headers, 0, len);
    // memset(data, 0, len);
    char * method = calloc(len,0 );	
    char * url = calloc(len, 0);	
    char * headers = calloc(len,0);	
    char * data = calloc(len,0 );
...
void *__http_server_listen_threader__(void *vp_arg)
{
    ...
    // CANNOT FREE BECAUSE SEPERATE THREAD
    // free(out.data);
    // free(out.headers);
    // free(out.method);
    // free(out.url);
...
int http_serve_static(int socket, char *filename, char *headers)
{
    ...
    if (fp)
    {
        ...
        {
            printf("Malloc Error"); fflush(stdout);
            exit(EXIT_FAILURE);
        }
        fclose(fp);
    }
...
http_header_map_t http_headers_format(char *string)
{
    ...
    printf("Error: Too many headers"); fflush(stdout);
    exit(1);
ret:
...
char *http_headers_map(http_header_map_t hmap, char *cmp)
{
